...
public class EntityPropertyRegistryImpl implements EntityPropertyRegistry {
    ...
    public void registerTypes(PacketFactory packetFactory, LegacyComponentSerializer textSerializer, TaskScheduler taskScheduler) {
        ...
        register(new DummyProperty<>("player_knockback_sound_name", getCompatibleVillagerNoSound()));
    ...
    private Sound getCompatibleVillagerNoSound() {
        try {
            // Try the modern sound name first
            return Sound.valueOf("ENTITY_VILLAGER_NO");
        } catch (IllegalArgumentException e) {
            try {
                // Fallback to older sound name
                return Sound.valueOf("VILLAGER_NO");
            } catch (IllegalArgumentException ex) {
                // If both fail, use a safe fallback sound that exists in all versions
                return Sound.valueOf("ENTITY_VILLAGER_AMBIENT");
            }
        }
    }
}
